...
def diabetes_predict():
    ...
    Age_select = st.sidebar.selectbox("Age", ['Select', 'Less than 40', '40-49', '50-59', 'Greater than 60'], index=0)
    Gender_select = st.sidebar.selectbox("Gender", ['Select', 'Male', 'Female'], index=0)
    Family_Diabetes_select = st.sidebar.selectbox("Family Diabetes History", ['Select', 'No', 'Yes'], index=0)
    highBP_select = st.sidebar.selectbox("High Blood Pressure", ['Select', 'No', 'Yes'], index=0)
    PhysicallyActive_select = st.sidebar.selectbox("Physical Activity Level", ['Select', 'Not at all', 'Less than Half hr.', 'More than Half hr.', '1 hr. or more'], index=0)
    BMI = st.sidebar.number_input("BMI", min_value=0.0, max_value=100.0, value=0.0)
    Smoking_select = st.sidebar.selectbox("Smoking", ['Select', 'No', 'Yes'], index=0)
    Alcohol_select = st.sidebar.selectbox("Alcohol Consumption", ['Select', 'No', 'Yes'], index=0)
    Sleep = st.sidebar.number_input("Sleep (hours)", min_value=0, max_value=12, value=0)
    SoundSleep = st.sidebar.number_input("Sound Sleep (hours)", min_value=0, max_value=12, value=0)
    RegularMedicine_select = st.sidebar.selectbox("Regular Medicine", ['Select', 'No', 'Yes'], index=0)
    JunkFood_select = st.sidebar.selectbox("Junk Food Consumption", ['Select', 'Occasional', 'Often', 'Very Often'], index=0)
    Stress_select = st.sidebar.selectbox("Stress Level", ['Select', 'Not at all', 'Sometimes', 'Very Often', 'Always'], index=0)
    BPLevel_select = st.sidebar.selectbox("Blood Pressure Level", ['Select', 'Normal', 'Low', 'High'], index=0)
    Pregancies = st.sidebar.number_input("Pregnancies (0-3)", min_value=0, max_value=3, value=0)
    UriationFreq_select = st.sidebar.selectbox("Urination Frequency", ['Select', 'Not much', 'Very often'], index=0)
